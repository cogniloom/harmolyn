...
1. `reset_sandbox` removes and recreates the container from the _existing_ `agenthub-sandbox:latest`
   image — it does **not** rebuild the image. Stale image → stale container after reset.
   Fix path: run `deploy_agenthub` first (triggers `build_sandbox.sh`), then `reset_sandbox`.
...
- `allowed_actions`: `reset_sandbox`, `deploy_agenthub`, `update_agent`, `create_agent`,
  `read_logs`, `task_note`.
