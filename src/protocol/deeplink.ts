...
/** Server-join metadata extracted from an invite, including the owner peer to dial. */
 ...
 */
export function parseInviteMetadata(raw: string): InviteMetadata {
...
export function parseJoinDeepLink(raw: string): DeepLink {
  ...
  switch (parsed.protocol.toLowerCase()) {
    ...
    case "xorein:":
      ...
      if (parsed.hostname.toLowerCase() === "join") return parseAetherJoinDeepLink(parsed);
...
function parseAetherJoinDeepLink(parsed: URL): DeepLink {
...
function parseXoreinInviteDeepLink(parsed: URL): DeepLink {
...
function parseXoreinInvitePayload(rawInvite: string): { serverId: string } {
