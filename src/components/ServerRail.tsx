...
export const ServerRail: React.FC<ServerRailProps> = ({ servers, activeServerId, connectionState, onSelectServer, onCreateServer, showExplore = true, homeBadge = 0 }) => {
  ...
  return (
    <div className="w-[70px] shrink-0 bg-bg-0 flex flex-col items-center py-3 gap-2 overflow-hidden border-r border-white/5 z-20 h-full" role="navigation" aria-label="Spaces" data-appearance-rail>
      {/* Home Button */}
      <div className="group relative flex shrink-0 flex-col items-center cursor-pointer">
          <motion.button 
      ...
      <div className="w-8 h-px shrink-0 bg-white/10"></div>

      <div className="flex min-h-0 w-full flex-1 flex-col items-center gap-3 overflow-y-auto overflow-x-hidden py-1 no-scrollbar" aria-label="Joined Spaces">
        {normalizedServers.map((server) => {
          ...
          return (
          <div key={server.id} className="group relative flex shrink-0 flex-col items-center cursor-pointer">
            <motion.button
               whileHover={{ opacity: 0.9 }}
               whileTap={{ opacity: 0.8 }}
      ...
      <div className="flex shrink-0 flex-col items-center gap-2">
        ...
        {showExplore && (
          <motion.button
            ...
            className={`w-[44px] h-[44px] rounded-full flex items-center justify-center transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed ${activeServerId === 'explore' ? 'bg-primary text-bg-0' : 'bg-white/5 text-primary/80 hover:text-primary hover:bg-primary/10'}`}>
            <Compass size={20} />
          </motion.button>
        )}

        <div
          className="flex flex-col items-center gap-1 px-2 pt-1 text-center"
          role="status"
        ...
        >
          ...
          <div className="max-w-[62px] truncate text-[8px] font-bold tracking-[0.14em] text-white/60">{connectionState.label}</div>
        </div>
      </div>
    </div>
  );
