...
describe("MediaLightbox", () => {
  ...
  it("pans with ordinary scroll and zooms with Ctrl+wheel, clamped to 0.5x..3x", () => {
    const onClose = vi.fn();
    const { getByRole } = render(
      <MediaLightbox src="https://example.com/photo.png" alt="Photo" onClose={onClose} />,
    );
    const dialog = getByRole("dialog");
    const scroller = dialog.querySelector(".media-lightbox-content")!;

    expect(screen.getByText("100%")).toBeTruthy();

    // Ordinary scrolling must not steal panning from a magnified image.
    ...
    for (let i = 0; i < 20; i += 1) fireEvent.wheel(scroller, { deltaY: -100, ctrlKey: true });
    expect(screen.getByText("300%")).toBeTruthy();

    // Many scroll-downs clamp at 0.5x (50%).
    for (let i = 0; i < 30; i += 1) fireEvent.wheel(scroller, { deltaY: 100, ctrlKey: true });
