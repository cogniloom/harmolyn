...
vi.mock('@/components/ChannelRail', () => ({ ChannelRail: ({ connectedVoiceChannelId, onJoinVoice }: {
  connectedVoiceChannelId: string | null; onJoinVoice: (id: string) => void;
}) => <div><output data-testid="voice-channel">{connectedVoiceChannelId ?? ''}</output><button onClick={() => onJoinVoice('lounge')}>Rail join</button></div> }));
...
describe('quick switcher voice integration', () => {
  ...
  it('requires identity before the quick switcher can join voice', async () => {
    ...
    expect(live.join).not.toHaveBeenCalled();
    expect(screen.getByTestId('auth-step')).toHaveTextContent('register');
  });
