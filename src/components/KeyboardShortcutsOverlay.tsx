...
export const KeyboardShortcutsOverlay: React.FC<KeyboardShortcutsOverlayProps> = ({ onClose }) => {
  ...
  return (
    <div className="responsive-overlay-scroll fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div
        className="flex max-h-full min-h-0 w-full max-w-[560px] flex-col overflow-hidden rounded-r3 border border-stroke glass-card"
      ...
      >
        {/* Header */}
        <div className="flex-shrink-0 border-b border-white/5 p-4 sm:p-5">
          <div className="flex items-center justify-between mb-4">
            <div className="flex min-w-0 items-center gap-2">
              <Command size={18} className="shrink-0 text-primary" />
              <h2 id="keyboard-shortcuts-title" className="text-title font-semibold text-text-primary">KEYBOARD // SHORTCUTS</h2>
            </div>
            <button onClick={onClose} aria-label="Close keyboard shortcuts" className="touch-target flex shrink-0 items-center justify-center rounded-full border border-stroke-subtle text-text-secondary transition-all hover:border-primary hover:text-primary glass-panel focus-ring">
              <X size={16} />
            </button>
          </div>
          <div className="relative">
            ...
            <input
              ...
              autoFocus
              className="h-11 w-full rounded-full border border-stroke-subtle bg-surface-dark pl-9 pr-4 text-body text-text-primary transition-colors placeholder:text-text-disabled focus:border-stroke-primary focus:outline-none"
            />
          </div>
        </div>
              ...
              <div key={cat}>
                ...
                <div className="space-y-1.5">
                  {catShortcuts.map((shortcut, i) => (
                    <div key={i} className="flex min-h-11 flex-col items-start gap-2 rounded-r1 px-3 py-2 transition-colors hover:bg-white/[0.03] sm:flex-row sm:items-center sm:justify-between">
                      <span className="break-words text-body text-text-secondary">{shortcut.description}</span>
        ...
        {/* Footer */}
        <div className="flex flex-shrink-0 flex-wrap items-center justify-center gap-x-4 gap-y-2 border-t border-white/5 px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] text-[10px] text-text-disabled sm:py-4">
